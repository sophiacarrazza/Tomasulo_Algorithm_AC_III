...
class TomasuloCore:
    ...
    def _issue(self):
        """Fase de despacho de instruções (superescalar: até 2 por ciclo, mas para no branch)"""
        if self.flush_needed:
            return
        max_issue_per_cycle = 2
        issued = 0
        while issued < max_issue_per_cycle:
            ...
            if available_rs:
                ...
                issued += 1
                if is_branch:
                    # Após emitir um branch, pare o despacho superescalar
                    if predicted_taken:
    ...
    def _commit(self):
        ...
        while commit_count < max_commit_per_cycle:
            ...
            if rob_entry.ready and rob_entry.instruction:
                ...
                rob_entry._init_() # Reseta a entrada do ROB para 'Empty'
    ...
    def _flush_pipeline(self):
        ...
        while current_idx != self.rob.tail:
            ...
            if entry_to_flush.state != 'Empty':
                ...
                # Limpa a entrada
                entry_to_flush._init_() # Reseta para o estado inicial
